...
//   in a cube configuration around the listener’s head. HRTF-based rendering is
//   enabled.
...
typedef struct gvr_audio_context gvr_audio_context;
...
// This method must be called from the main thread at a regular rate. It is used
// to execute background operations outside of the audio thread.
//
// @param api Pointer to a gvr_audio_context.
...
class AudioApi {
 public:
  ...
  void SetHeadRotation(const Mat3f& rotation) {
  ...
  // Sets the room properties describing the dimensions and surface materials of
  // a given room. For more information, see gvr_audio_set_room_properties().
