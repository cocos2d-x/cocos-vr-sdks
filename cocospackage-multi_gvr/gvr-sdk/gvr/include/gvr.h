...
// This is the Google VR C API. It supports clients writing VR experiences
// for head mounted displays that consist of a mobile phone and a VR viewer.
//
...
//     // Java GvrLayout object via GvrLayout.getGvrApi().getNativeGvrContext().
//     gvr_context* gvr = ...;
//     #else
...
// Destroys a gvr instance.  The parameter will be nulled by this operation.
//
...
//     be populated with the recommended render params and resized if necessary.
...
// the lenses, and renders it to the screen. It performs post-frame
// operations like distortion correction and more.  Should be called after
// the app renders its frame. Note that the target that is referenced by the
// |target_id| will be post-processed to the display, and the original target
...
// @param index Index of the render params entry to update. If the |params_list|
//     size is equal to the index, a new params entry will be added. The
//     |params_list| size must *not* be less than the index value.
...
// @param framebuffer_size The specified size of the offscreen framebuffer to
//     create.
...
//
// @param gvr Pointer to the gvr instance which will pause tracking and disable
//     sensors.
void gvr_pause_tracking(gvr_context* gvr);

// Resumes head tracking, re-enables all sensors.
//
// @param gvr Pointer to the gvr instance which will resume tracking.
void gvr_resume_tracking(gvr_context* gvr);

// Resets head tracking.
//
// @param gvr Pointer to the gvr instance which will reset tracking.
...
// @param viewer_profile_uri A string that contains either the shortened URI
//     or the standard URI representing the viewer profile that the app should
//     be using. If the valid viewer profile can be found on the device, the
//     URI that is passed in will be ignored and nothing will happen. Otherwise,
...
//     only until the next use of the |gvr| context. The string should be copied
//     immediately if persistence is required.
...
// destruction. It can only be created via a |GvrApi| instance, and its validity
// is tied to the lifetime of that instance.
class RenderParamsList {
 public:
  ...
  // For more information, see gvr_get_recommended_render_params().
  void SetToRecommendedRenderParams() {
    gvr_get_recommended_render_params_list(context_, params_list_);
  }

  // For more information, see gvr_get_screen_render_params().
...
// Convenience C++ wrapper for a GVR offscreen framebuffer handle.
// Note that this wrapper does *not* automatically free the framebuffer
// resources upon destruction; GvrApi will do that automatically when the
// context is destroyed. The client may explicitly free the framebuffer by
// calling Reset(). Also note that this wrapper can only be created via a
// |GvrApi| instance, and its validity is tied to the lifetime of that instance.
class OffscreenFramebufferHandle {
 ...
 private:
  ...
  OffscreenFramebufferHandle(gvr_context* context, Sizei size)
      : context_(context),
        handle_(gvr_create_offscreen_framebuffer(context, size)) {}
...
// Example API usage:
//
//     // Functionality supplied by the application in the example below has the
//     // "app-" prefix.
//     #ifdef __ANDROID__
//     // On Android, the gvr_context should almost always be obtained from the
//     // Java GvrLayout object via GvrLayout.getGvrApi().getNativeGvrContext().
...
class GvrApi {
 public:
  ...
  //
  // @param size The specified size of the OffscreenFramebufferHandle to create.
  // @return A managed warpper for an offscreen framebuffer handle.
  //     Note: The validity of the returned object is closely tied to the
  //     lifetime of the member gvr_context. The caller is responsible for
  //     ensuring correct usage accordingly.
  OffscreenFramebufferHandle CreateOffscreenFramebuffer(Sizei size) {
    return OffscreenFramebufferHandle(context_, size);
  }
  ...
  // For more information, see gvr_get_eye_window_bounds().
